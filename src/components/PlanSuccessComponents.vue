...
export default {
    ...
    data() {
        ...
        async Showplans() {
            try {
                const respone = await axios.get('https://node-2i60.onrender.com/api/select-plan-success');
        ...
        async Searchplan() {
            ...
            fetch(`https://node-2i60.onrender.com/api/search-plan-success?${params.toString()}`)
                .then((respone) => respone.json())
                .then((data) => {
                    this.plans = data;
                })
